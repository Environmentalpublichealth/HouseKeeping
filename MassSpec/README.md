...
* `Count-UserLCMSperMonth.ps1` - count the number of LC/MS batch folders and `.raw` files in one user folder.
* `Count-MonthlyLCMSbyUser.ps1` - count the number of batch folders and `.raw` files in all users in a specified month.
* `Count-UserGCMSperMonth.ps1` - count the number of GC/MS '.qgd' files in one user folder.
* `Count-MonthlyGCsamplesByUser.ps1` - count the number of `.qgd` files in all users in a specified month.
...
#### Count one User for all months
Now the script is ready to run as:
```bash
.\Count-UserGCMSperMonth.ps1 "C:\GCMSsolution\Data\FolderName"
```
...
...
```
The GCMS data files are not organized by batch folder, so we are only counting how many samples by month for one person. 

#### Count one Month for all users
To loop over all users and count for a single month, run this script:
```bash
.\Count-MonthlyGCsamplesByUser.ps1 "C:\GCMSsolution\Data" "2026-08"
```
Change the "YYYY-MM" to the month you want, follow the exact same year-month format. The output should be:
```txt
User    samples
AAA      0
BBB      40
...
Now the script is ready to run as:
```bash
.\Count-UserLCMSperMonth.ps1 "C:\TraceFinderData\Projects\FolderName"
```
Change the 'FolderName' to the user folder you want to count. The output should be: 
```txt
...
To loop over all users and count for a single month, run this script:
```bash
.\Count-MonthlyLCMSbyUser.ps1 "C:\TraceFinderData\Projects" "2026-08"
```
Change the "YYYY-MM" to the month you want, follow the exact same year-month format. The output should be:
```txt
User   batches   samples
AAA      0        0
BBB      1        13
...
```
